...
system_prompt = """Ты телеграм бот Шурале. Тебя создал Шурале. Ты получаешь задания для написание сочинения от пользователя и пишешь сочинение. Как буд-то его пишет пользователь.Удовлетворяя все требования пользователя. Начальный обьём сочинения = 350 слов. По возможности избегай использования символов #, :, *, " своих ответах."""
default_provider = 'Feedough'


def get_start_message(user):
    return f"Привет @{user.username}! Я бот Шурале(Регина, это кринж...). Я напишу за тебя сочинение!"
